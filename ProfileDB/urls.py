...
auth_url_patterns = [  
  ...
  url(r'^activate/(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]{1,20})/(?P<uidb64>[0-9A-Za-z_\-]+)/$', views.Activate, name='activate'),  
] 
...
curd_url_patterns = [
  ...
  url(r'^ajax/update_profile/$', views.UpdateProfile, name='UpdateProfile')
]

base_url_patterns = [url(r'^{}/$'.format(category), 
                         views.__getattribute__(category.capitalize() + 'View'),
                          name=category)
                    for category in views.categories]

urlpatterns = auth_url_patterns + curd_url_patterns + base_url_patterns 
